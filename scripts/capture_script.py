...
def _setup_commands(plan: CaptureScriptPlan) -> list[str]:
    ...
    if plan.hide_hud:
        cmds += [
            ...
            # pouco na tela". Round 4 (06/05 noite): viewmodel_offset_x 0
            # ainda não suficiente — push mais. Tentando offset_x -2.0 +
            # viewmodel_fov maior pra weapon visualmente maior.
            #
            # Análise: target visível columns 471-808 (337px wide). Default
            # viewmodel x=2.5 → weapon column ~1100 (fora). offset_x 0 →
            # column ~860 (borderline visível). offset_x -2.0 → column ~620
            # (clear visible, próximo crosshair em 640).
            #
            # Trade-off: offset_x muito negativo pode sobrepor visualmente
            # com crosshair. -2.0 é range max negativo de CS2, weapon
            # ainda fica deslocado lateralmente (não exatamente em cima do
            # crosshair).
